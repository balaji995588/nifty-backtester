...
# MACD signal: 1 when MACD above signal line
df['Signal_MACD'] = 0
df.loc[df['MACD'] > df['MACD_Signal_Line'], 'Signal_MACD'] = 1

# Combined signal: SMA trend AND MACD momentum both confirming
df['Signal_Combined'] = 0
df.loc[(df['SMA20'] > df['SMA50']) & (df['MACD'] > df['MACD_Signal_Line']), 'Signal_Combined'] = 1

# Returns for both new strategies
df['Strategy_Return_MACD'] = df['Market_Return'] * df['Signal_MACD'].shift(1)
df['Strategy_Return_Combined'] = df['Market_Return'] * df['Signal_Combined'].shift(1)

# Cumulative returns
df.dropna(inplace=True)
df['Cumulative_MACD'] = (1 + df['Strategy_Return_MACD']).cumprod()
df['Cumulative_Combined'] = (1 + df['Strategy_Return_Combined']).cumprod()

# ---- FINAL COMPARISON CHART ----
plt.figure(figsize=(14, 7))
plt.plot(df['Cumulative_Market'], color='#1A1A2E', linewidth=2, label='Buy & Hold')
plt.plot(df['Cumulative_Strategy'], color='#D97706', linewidth=1.5, label='SMA Crossover')
plt.plot(df['Cumulative_RSI'], color='#4F46E5', linewidth=1.5, label='RSI Filtered', linestyle='--')
plt.plot(df['Cumulative_MACD'], color='#DC2626', linewidth=1.5, label='MACD', linestyle='--')
plt.plot(df['Cumulative_Combined'], color='#059669', linewidth=1.5, label='Combined SMA+MACD', linestyle=':')
plt.axhline(y=1.0, color='#6B7280', linewidth=0.8, linestyle='--', label='Starting Capital')
plt.title('All Strategies vs Buy & Hold — Cumulative Returns (2023–2024)')
plt.xlabel('Date')
plt.ylabel('Growth of ₹1 Invested')
plt.legend()
plt.grid(True, alpha=0.3)
